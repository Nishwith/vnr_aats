...
const StaffManager = () => {
  ...
  // Reset to page 1 when search or department filters change
  useEffect(() => {
    setCurrentPage(1);
  ...
  useEffect(() => {
    refreshData();
  }, [currentPage, searchQuery, filterDepartment, startDate, endDate]);

  const refreshData = async () => {
    setLoading(true);

    // Fetch ONLY paginated staff
    const { data: fetchedStaff, count } = await getPaginatedStaff(
      currentPage,
      itemsPerPage,
      searchQuery,
      filterDepartment,
    ...
    const [fetchedWorkplaces, todaysAtt, allAtt] = await Promise.all([
      getWorkplaces(),
      getTodaysAttendanceRecords(),
      getAllAttendanceLogs(),
    ]);
    ...
    if (!formState.workplaceId && fetchedWorkplaces.length > 0) {
      setFormState((prev) => ({
        ...prev,
        workplaceId: fetchedWorkplaces[0].id,
      }));
    }

    // Calculate Global Attendance Percentage & Today's presence
    const stats = {};
  ...
  const handleSave = async (e) => {
    ...
    if (!formState.workplaceId) {
      alert(
        "Please select an Assigned Workplace! If the list is empty, navigate to 'Workplace Settings' to create one first.",
      );
      return;
    }
    if (editingStaff) {
  ...
  const toggleAttendance = async (dateStr, slot, currentValue) => {
    ...
    await updateAttendanceRecord(attendanceViewRow.id, dateStr, {
      [slot]: newValue,
    });
    await loadAttendanceHistory(attendanceViewRow.id);
    await refreshData();
  };

  // Client-side filter for Attendance status (since it relies on calculated stats)
  const filteredStaff = staff.filter((s) => {
    let matchAtt = true;
    if (filterAttendance === "absent")
      matchAtt = (attendanceStats[s.id]?.todayAttScore || 0) === 0;
  ...
  const handleExportCSV = () => {
    ...
    let csvContent =
      "Employee Name,Employee ID,Role,Branch,Date Range Tracking Days,Days Present,Overall Percentage\n";
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Manage personnel, roles, and attendance records.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={handleExportCSV}
            className="flex items-center gap-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-5 py-2.5 rounded-xl transition-all font-bold border border-emerald-200"
          >
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  required
                  value={formState.workplaceId}
                  onChange={(e) =>
                    setFormState({ ...formState, workplaceId: e.target.value })
                  }
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                >
            ...
            {isAdding && (
              <div className="bg-amber-50 text-amber-800 p-3 rounded-xl border border-amber-200 text-sm">
                ...
                The default password for new staff is set to{" "}
                <code className="bg-amber-100 px-1 py-0.5 rounded text-amber-900 font-mono font-bold">
                  vnrvjiet
                </code>
                . They can change it in their profile later.
              </div>
            )}
            <div className="pt-2 flex gap-3">
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-700 text-white font-medium px-6 py-2.5 rounded-xl shadow-lg shadow-blue-500/30 transition-all flex items-center gap-2"
              >
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">
                  {attendanceViewRow.name} ({attendanceViewRow.employeeId})
                </p>
              </div>
              <button
                onClick={() => setAttendanceViewRow(null)}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-3">
                {attendanceRecords.map((rec, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-200 hover:border-blue-300 transition-colors"
                  >
                    ...
                    <div className="flex gap-4 mt-3 sm:mt-0">
                      ...
                      <button
                        onClick={() =>
                          toggleAttendance(rec.date, "morning", rec.morning)
                        }
                      ...
                      <button
                        onClick={() =>
                          toggleAttendance(rec.date, "evening", rec.evening)
                        }
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-col">
                        ...
                        <div className="text-sm font-mono text-slate-500 mt-0.5">
                          {s.employeeId} • Branch: {s.department}
                        </div>
                        <div className="text-xs mt-1.5 font-semibold">
                          ...
                          <span
                            className={
                              stat.todayAttScore === 1
                                ? "text-emerald-600"
                                : stat.todayAttScore === 0.5
                                  ? "text-amber-500"
                                  : "text-rose-500"
                            }
                          >
                      ...
                      {s.photoBase64 ? (
                        <div className="flex items-center gap-3">
                          <img
                            src={s.photoBase64}
                            alt="Reference"
                            className="w-12 h-12 rounded-full object-cover border-2 border-green-200 bg-green-50"
                          />
                          <span className="text-xs font-medium text-green-700 flex items-center gap-1 bg-green-100 px-2 py-1 rounded-lg">
                            <Camera size={12} /> Configured
                          </span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-medium text-amber-700 flex items-center gap-1 bg-amber-100 px-2 py-1 rounded-lg">
                            Missing
                          </span>
                        </div>
                      )}
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-end gap-2 flex-wrap max-w-[200px] ml-auto">
                        <button
                          ...
                          className="px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg text-sm font-medium transition flex justify-center items-center gap-1.5 w-[calc(50%-0.25rem)]"
                          title="Attendance"
                        >
                        ...
                        <button
                          onClick={() => openEdit(s)}
                          className="px-3 py-1.5 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-lg text-sm font-medium transition flex justify-center items-center gap-1.5 w-[calc(50%-0.25rem)]"
                          title="Edit Details"
                        >
                          <Edit size={14} /> Edit
                        </button>
                        ...
                        {s.photoBase64 ? (
                          <button
                            ...
                            className="px-3 py-1.5 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-lg text-sm font-medium transition flex justify-center items-center gap-1.5 w-[calc(50%-0.25rem)]"
                            title="Remove Photo"
                          >
                            <Trash2 size={14} /> Pic
                          </button>
                          ...
                          <label
                            className="cursor-pointer px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-sm font-medium transition flex justify-center items-center gap-1.5 w-[calc(50%-0.25rem)]"
                            title="Upload Photo"
                          >
                            <Camera size={14} /> Pic
                            <input
                              ...
                              accept="image/*"
                              className="hidden"
                              onChange={(e) => handlePhotoUpload(e, s.id)}
                            />
                          </label>
                        )}
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-100 text-slate-600 hover:bg-red-100 hover:text-red-700 rounded-lg text-sm font-medium transition flex justify-center items-center gap-1.5 w-[calc(50%-0.25rem)]"
                          title="Delete Staff"
                        >
              ...
              {!loading && filteredStaff.length === 0 && (
                <tr>
                  ...
                  >
                    ...
                    {totalCount === 0
                      ? "No staff registered yet. Add one to get started."
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between bg-slate-50">
            <button
              ...
              className="flex items-center gap-1 px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-xl hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed font-medium text-sm transition-colors shadow-sm"
            >
